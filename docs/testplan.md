...
| OPEN-31 | **[PLANNER] The ingest retrieval path needs re-ruling before slice 1 is built.** The handover's §3 seeds `filer` from `company_tickers.json` and crawls submissions per CIK. **Measured: that file holds 8,049 current CIKs and no dead ones** - Lehman, Sears, Bed Bath & Beyond and Enron are all absent, Apple present as control. A universe seeded from it **cannot** contain a company that stopped trading, so the very question the slice exists to answer comes back survivorship-filtered **while looking computed**. **Proposed:** `filer` + `filing` from **`submissions.zip`** (all filers, full history, one request, ~1.56 GB, nightly); `filer_ticker` from the ticker files as the **current-day crosswalk only** - which is what ruling 5 already says they are for. See `F-next/ticker-file-seeding-reintroduces-survivorship`. | Ingest slice 1's correctness under ruling 5. |
