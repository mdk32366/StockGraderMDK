...
| OPEN-56 | **Can provisioned storage be raised in place, and does it grow automatically?** Fly bills **provisioned**, not used, and documents storage growth as monitored/managed. **Which mistake is cheap depends on this**: if it can be raised later, under-provisioning is recoverable and over-provisioning is money spent on empty space. **Establish before the owner picks a figure.** Costs nothing. | OPEN-53's disk figure. |
| OPEN-57 | **The RAM question, measured before it is paid for.** Basic is Shared-2x with **1 GB**; at ~114 GB `fact_one_per_filing` alone is ~42 GB of index that the initial load has to **build**. Load enough quarters locally against constrained memory and time it. **State the caveat WITH the result: a local host's page cache is not constrained even when Postgres's is, so the simulation is OPTIMISTIC. It can prove Basic is inadequate; it cannot prove Basic is sufficient.** A green result here is weaker than it looks. | OPEN-53's plan tier. |
