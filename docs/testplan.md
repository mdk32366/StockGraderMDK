...
| ~~OPEN-65~~ | **CLOSED 2026-09-25 - NOT by measuring, and that is the point.** ~45 min/quarter, ~34 h for 45, is **acceptable because the load is restartable**, and the Planner required that property be confirmed rather than assumed. **Confirmed (F-034):** quarters commit independently (each load is one transaction in one process); re-running a completed quarter is a **no-op** and is *faster* - 63 s against 148 s - because every row conflicts away rather than inserting, and the speed difference is itself the evidence; and a discontiguous window reports its gaps, `2015q2..2026q2 quarters=2 gaps=43`, where **2 + 43 = 45**, the ruled window, derived by the view rather than asserted. **The proposed `--defer-indexes` measurement was REFUSED** on `F-next/a-measurement-that-decides-nothing`: nothing downstream waits on the load and the result is identical either way, so no decision turned on it (**F-035**). `--defer-indexes` is built, tested and unused. | - |
| OPEN-66 | **[OWNER/PLANNER] §11.1 / §5.4 — the R&D treatment. SESSION SCHEDULED 2026-09-25 by the owner; order at `docs/planner/ORDER-Planner-2026-09-25-StockGraderMDK-rd-treatment-session.md`.** **This is P-13 given a date.** §11.1 was deferred to *"its own session"* on day one and no session was ever scheduled; the scoring track has not moved in three days and this is the product's critical path. **The question:** is R&D left as an expense in the year incurred, or capitalised and written down over an assumed useful life? Leaving it as an expense **systematically penalises the businesses a growth-quality model should rank highest**; capitalising **inserts an assumption nobody discloses** — no filing states the useful life of research, so choosing three, five or eight years *is* the ranking for that population. **Three settlements, in the Planner's preference order:** (1) rule that v1 does not claim to handle research-intensive companies well and state the limitation in the output — cheapest, honest, reversible; (2) compute both and report the rank correlation, **recommended as the opening move** because the facts now exist; (3) capitalise with a stated life and a sensitivity note. **Step 0 before any of them:** verify the store actually holds `ResearchAndDevelopmentExpense` — option 2 rests on that claim and **nobody has checked it**, which is **P-12's shape**, *a ruling that depends on a figure waits for the figure*. **Note the circularity** option 2 carries: it needs enough scoring built to run twice, and scoring is what the gate blocks — if that binds, option 1 now and option 2 on revisit. **The session must produce a D-entry with a stated revisit condition**; a ruling with no revisit condition is how §11.1 became P-13. | The GQS §13 completeness gate (two rows below); the reinvestment rate, **gated by §11.1** per `gqs-source-map.md`, and through it two of the five scoring blocks. **Ingest is unaffected.** |
...
| §13 condition | State |
|---|---|
| Any §11 question unratified | **OPEN - SESSION SCHEDULED 2026-09-25** (OPEN-66). 11.1 deferred on day one (ruling 12), undated until now - that is P-13. **11.2, 11.3, 11.4 ruled.** |
| §5.4 R&D treatment unresolved | **OPEN - the same question as 11.1**, and it moves when 11.1 moves. Same session, same ruling, same date. |
