...
| OPEN-27 | **Migration 0001 cannot run as `stockgrader_app`, and every `schema_admin` we have is compromised.** A-017 established a `writer` is refused `CREATE TABLE` - **by design** (D-021, D-031). So 0001 needs a `schema_admin` on `kzpwm0j1dm204nv3`, and the only two are `fly-user` and `stockgradermdk`, **both compromised 2026-09-22 and both carried across by the restore.** **The roles question and the pooler question meet here.** **Resolution, from OPEN-26:** create a clean `schema_admin` for migrations, set its password in the dashboard, store it, **then** drop the compromised pair - in that order, so the recovery and migration paths are never both absent. | Migration 0001's run phase; OPEN-25's ordering. |
| OPEN-28 | **The migration runner does not exist.** D-021 requires numbered forward-only SQL applied by **a runner we own**, each migration in its own transaction under an advisory lock, followed by its verification query. **0001 and its verification are written and proven; the runner is not built.** Until it is, applying a migration is a human running `psql -f`, which is D-021's shape without D-021's guarantees - no ledger write, no checksum, no enforced ordering. `schema_migration` exists and its `sha256` column is a placeholder (`SET-BY-RUNNER`). | Any claim that D-021 is implemented rather than designed. |
