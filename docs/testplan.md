...
| OPEN-53 | **[OWNER] 45 quarters does not fit - 124.4 GB against a 15 GB cluster.** One quarter measured at **1,814 MB** (3.37M facts, 565 bytes/row incl. indexes); scaled by actual archive bytes for the 45-quarter window (ratio **70.22**, not 45 - 2026q2 is among the smallest). **Naive x45 understates by 56%.** Per D22 §4: report and stop. **New information for the preference order: indexes are 61% of storage and `fact_one_per_filing` alone is 42% of everything** - simultaneously the largest object and the least negotiable, since it is what makes overwrite-on-amendment unavailable. The four smaller indexes (277 MB, 15%) are performance choices rather than correctness ones. | The fact slice's load step. |
| OPEN-54 | **0002 does not cover the `fact` table** - `source_fetch_id` was added to `filer`, `filer_ticker` and `filing` only, and **verification A5 enumerates those same three**, so it passes while every fact row is unprovenanced. **This is the exact gap D16 §2.1 ordered 0002 to close.** Requires **0003**, not an amendment - and no fact data exists anywhere yet, which is the same cheap-now-impossible-later window 0002 was ordered inside. | Provenance covering the data that actually has volume. |
