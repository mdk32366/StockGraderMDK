...
| OPEN-57 | **MEASURED 2026-09-25 - and the measurement CANNOT answer the question. Still open.** Under Basic-shaped Postgres memory (`shared_buffers=128MB`, `maintenance_work_mem=64MB`) a **1,075 MB** `fact_one_per_filing` built in **21.3 s** over 5.6M facts - extrapolating to ~**0.3 h** for 42 GB. **So the external sort is not the constraint.** But the host has **31.5 GB RAM** and the 1,089 MB table sat entirely in an **unconstrained OS page cache**; on Basic, 1 GB is the *whole machine*. **The simulation was optimistic by ~31x on the one resource that matters.** Per D25 §5.2 it could have proved Basic inadequate; it did not, so **it proves nothing** - a weaker outcome than *Basic looks fine*. **What would answer it:** a 1 GB-total constrained host, or a real Basic cluster. **Only the external sort is off the risk list.** | OPEN-53's plan tier. |
