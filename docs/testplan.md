...
| OPEN-36 | **[PLANNER] The fact-slice source trade-off, established and unresolved.** **companyfacts** is nightly but carries **no dimensions and no per-fact entity** - it would make `dimensions` and `entity_cik` permanently trivial (D16 §3.1's trap, confirmed). **Financial Statement Data Sets** carry both - `segments` on **60.7%** of 3.6M rows, `coreg` on 61,122 - but are **quarterly with a ~50-day publication lag**. Neither dominates. Both carry amendment history with distinct accessions and filing dates. **Design is the Planner's call**; citing the ruling each source satisfies is the P-10 rule. | The fact slice's handover. |
