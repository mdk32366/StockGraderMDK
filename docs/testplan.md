...
| OPEN-39 | **Is FSDS `num.txt.coreg` resolvable to a CIK?** OPEN-36 rests on two pillars - dimensions and a per-fact entity - and this one is unestablished. `entity_cik` is `bigint REFERENCES filer`, but `coreg` may be a **label**, not a CIK. **Three possibilities, none assumed:** (1) it is a CIK - mapping direct; (2) a name resolvable against `sub.txt.aciks` - the mapping exists and **its failure rate is a number we need**; (3) free-text with no reliable path to a CIK - then FSDS gives a per-fact entity **marker** but not an **identifier**, and those 61,122 rows cannot populate `entity_cik NOT NULL`. **Under (3) the scheme-refusal rule means co-registrant facts are REFUSED** - a much narrower outcome than the ruling assumes, and one to rule on explicitly rather than let emerge as a rejection rate. **§2 still holds under (3) on dimensions alone, but the record must say which pillar it rests on.** | The fact-slice handover; `entity_cik`'s populability. |
